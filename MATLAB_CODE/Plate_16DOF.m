...
num_elements = size(code, 1);  % Number of elements
        ...
        if code(elem, i) ~= 0  % If the DOF is not restrained
            for j = 1:16
                if code(elem, j) ~= 0  % If the DOF is not restrained
                    ...
                    K(code(elem, i), code(elem, j)) = K(code(elem, i), code(elem, j)) + ke(i, j);
                end
            end
            % Assemble the global force vector
            F(code(elem, i), 1) = F(code(elem, i), 1) + fe(i, 1);
        end
    end
end
%-------------------------SOLVING DEFORMATION PROBLEM----------------------
Delta = inv(K)*F
midelem=fix(Ney/2)*Nex+fix(Nex/2)+1;
corXmid=(Nex/2-fix(Nex/2))*a; corYmid=(Ney/2-fix(Ney/2))*b;
Wmid=0;
for i=1:16
    if code(midelem, i)~=0
        Wmid=Wmid+double(subs(subs(N(i)* ...
            Delta(code(midelem,i)),x,corXmid),y,corYmid));
    end
end
wmidND=Wmid/(p0*Lx^4/d)
